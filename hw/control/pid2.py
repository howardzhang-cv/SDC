...
class PID(ControlLoop):
    ...
    def angle_difference(self, angle1, angle2):
        min_angle = min(abs(angle1 - angle2), 2*np.pi - abs(angle1 - angle2))
        return min_angle
    ...
    def calc_action(self, state):
        ...
        if self.total_error2 == 0:
            error = self.angle_difference(angle, cur_angle)
            if cur_angle + error == angle:
                self.angle = 1
            else:
                self.angle = -1
        ...
        error2 = self.angle_difference(angle, cur_angle)
        ...
        if self.debug:
            print("State: {}, Action: {}, Errors: {}".format([cur_angle, angle, error2], self.cur_waypoint, [kp2*self.last_error2, kd2*derror2, ki2 * self.total_error2]))
            #print("State: {}, Error: {}".format(state, [error1, error2]))
        return [action1/velocity_factor, action2]
...
if __name__ == "__main__":
    ...
    trajectory1 = [np.array([2., 1.]), np.array([2., 3.]), np.array([3.2, 3.]), 
                    np.array([3.7, 2.5]), np.array([3.7, 1.]), np.array([2, 1.]), np.array([2., 3.]), np.array([1., 3.]), np.array([1.,1.])]

    trajectory2 = [np.array([1., 2.]), np.array([2., 2.]), np.array([2, 3.]), 
                    np.array([3.2, 3.]), np.array([3.7, 2.5]), np.array([3.7, 1.]), np.array([2., 1.]), np.array([2., 2.]), np.array([1.,2.]), 
                    np.array([1., 3.]), np.array([2, 3]), np.array([2., 2.]), np.array([1., 2.]), np.array([1., 1.])]
    ...
    control_loop = PID(waypoints=trajectory2, dt=env.delta_time, threshold=0.1, debug=True)
